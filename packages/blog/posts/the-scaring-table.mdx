...
Ahh, it resolves the problem of the last row, but a new problem arises: the rounded corners are gone. It's because the `border-collapse` property is not compatible with `border-radius`.
...
It won't take too much effect to notice the background overflowed to the border. It originates from the incompatible of `border-collapse` and `border-radius`. (Which means `outline` is indeed a workaround, not a solution.)
...
A "good" new is that even had we chosen the workaround to hide the border of the last row, we will still come into this problem and even the same workaround.
