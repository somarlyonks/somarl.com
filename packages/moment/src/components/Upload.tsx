...
export default forwardRef<HTMLInputElement, IProps>(function Upload ({onUpload, onError, onStart, onFinish}, ref) {
    ...
    const handleUpload: ChangeEventHandler<HTMLInputElement> = async (event) => {
        ...
        try {
            ...
            const {url, fields}: {url: string, fields: Record<string, string>} = await fetch(`/api/s3?key=${filename}&accesskey=${encodeURIComponent(localStorage.getItem('S3_SECRET_ACCESS_KEY') || '')}`).then(r => r.json())
